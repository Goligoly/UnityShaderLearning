...
public class CascadedShadowmap : BaseShadowCamera
{
    ...
    protected Matrix4x4[] lightSpaceMatirx;
    ...
    void Start()
    {
        ...
        Shader.SetGlobalFloatArray("_CascadedDistance", mainCameraCascaded);
    }
    ...
    void Update()
    {
        ...
        for (level = 0; level < cascadedLevels; level++)
        {
            ...
            CalcMinObjectZ(); //计算所有物体在光线空间最小的Z值
            ...
            ShadowTextureRender(depthTexture[level], ref lightSpaceMatirx[level]);
        }
        Shader.SetGlobalMatrixArray("_CustomLightSpaceMatrix", lightSpaceMatirx);
    }
    ...
    protected void InitCascadedData()
    {
        ...
        lightSpaceMatirx = new Matrix4x4[cascadedLevels];
    }
}
