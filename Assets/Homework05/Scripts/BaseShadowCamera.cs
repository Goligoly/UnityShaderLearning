...
public class BaseShadowCamera : MonoBehaviour
{
    ...
    protected float minObjZ = float.MaxValue;
    ...
    protected void CalcMinObjectZ()
    {
        minObjZ = float.MaxValue;
        foreach (GameObject go in GameObject.FindObjectsOfType<GameObject>())
        {
            if (go.transform.GetComponent<MeshRenderer>() == null) continue;
            Bounds goBound = go.transform.GetComponent<MeshRenderer>().bounds;
            Vector3 lightSpaceCenter = shadowMapCam.transform.InverseTransformPoint(goBound.center);
            minObjZ = Mathf.Min(minObjZ, lightSpaceCenter.z - Mathf.Max(goBound.size.x, goBound.size.y, goBound.size.z));
        }
    }
    ...
    protected void ShadowTextureRender(RenderTexture renderTexture, ref Matrix4x4 lsm)
    {
