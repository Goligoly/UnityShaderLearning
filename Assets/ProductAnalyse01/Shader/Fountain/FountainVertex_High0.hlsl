...
VS_OUTPUT generateOutput(VS_INPUT input)
{
    ...
    output.v1 = _vs_TEXCOORD0;//MaintexBumpUVs
    ...
    output.v6 = _vs_TEXCOORD1;//CameraDir
...
VS_OUTPUT main(VS_INPUT input)
{
    ...
    (_u_xlat12 = dot(worldNormal.xyz, worldNormal.xyz));
    (_u_xlat12 = rsqrt(_u_xlat12));
    (worldNormal.xyz = (vec3_ctor(_u_xlat12) * worldNormal.xyz));
    ...
    (_u_xlat12 = dot(worldTangent.xyz, worldTangent.xyz));
    (_u_xlat12 = rsqrt(_u_xlat12));
    (worldTangent.xyz = (vec3_ctor(_u_xlat12) * worldTangent.xyz));

    float3 worldBitangent;
    (worldBitangent.xyz = (worldNormal.xyz * worldTangent.xyz));
    (worldBitangent.xyz = ((worldNormal.zxy * worldTangent.yzx) + (-worldBitangent.xyz)));
    (_u_xlat12 = (_in_TANGENT0.w * _unity_WorldTransformParams.w));
    (worldBitangent.xyz = (vec3_ctor(_u_xlat12) * worldBitangent.xyz));
    (_vs_TEXCOORD4.y = worldBitangent.x);
    (_vs_TEXCOORD4.w = _u_xlat0.x);
    (_vs_TEXCOORD4.x = worldTangent.z);
    (_vs_TEXCOORD4.z = worldNormal.y);
    (_vs_TEXCOORD5.x = worldTangent.x);
    (_vs_TEXCOORD6.x = worldTangent.y);
    (_vs_TEXCOORD5.z = worldNormal.z);
    (_vs_TEXCOORD6.z = worldNormal.x);
